...
length(unique(INTEGRADIV_birds_occ_df$Species))

# Only keep 50x50 assemblages ie cells:
temp_birds_sp_asb_50km <- dplyr::filter(INTEGRADIV_birds_occ_df,
                                        Grid == "50x50")

# Pivot wider so asb are columns:
birds_sp_asb_50km <- temp_birds_sp_asb_50km %>%
...
birds_phylogeny$tip.label <- gsub("_", " ", birds_phylogeny$tip.label)


# Check if all species in the occurrence df are in the phylogeny: YES :)
setdiff(unique(INTEGRADIV_birds_occ_df$Species),
        birds_phylogeny$tip.label)

# Check phylogeny only contains species in the occurrence df: YES :)
setdiff(birds_phylogeny$tip.label,
        unique(INTEGRADIV_birds_occ_df$Species))


# Save the birds phylogeny:
ape::write.tree(birds_phylogeny,
