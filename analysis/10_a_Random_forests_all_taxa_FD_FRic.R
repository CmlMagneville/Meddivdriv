...
locate.cells(cell_vect = rownames(rf_fric_birds_df),
             grid = grid_50km)
...
# Std Variable importance:
varimp_reptiles[[2]]
# Mean R-squared: 0.347773
varimp_reptiles[[3]]
# Sd R-squared: 0.01106378
varimp_reptiles[[4]]
...
ggplot2::ggsave(plot = varimp_plot_reptiles,
                filename = here::here("outputs",
                                      "varimp_FD_fric_50_REPTILES.pdf"),
                device = "pdf",
                scale = 1,
                height = 5000,
                width = 8000,
...
# Std Variable importance:
varimp_trees[[2]]
# Mean R-squared: 0.5033567
varimp_trees[[3]]
# Sd R-squared: 0.005911417
varimp_trees[[4]]
...
# Check types of variables
str(rf_fric_mammals_df)

# Check that diversity metric doesn't have NA:
rownames(rf_fric_mammals_df[which(is.na(rf_fric_mammals_df$ses) == TRUE), ])

# Change SES from names num to num:
rf_fric_mammals_df$ses <- as.numeric(rf_fric_mammals_df$ses)

# Set seed for randomisation:
set.seed(42)

# See if 300 mammals and mtry = 17 ok:
# Run the random forest model mtry = 17 and 300 mammals:
rf_mammals <- randomForest::randomForest(ses~.,
                                         data = rf_fric_mammals_df,
                                         ntree = 300,
                                         mtry = 17,
                                         importance = TRUE)
# ntree:
...
# mtry:
mtry <- randomForest::tuneRF(rf_fric_mammals_df[-ncol(rf_fric_mammals_df)],
                             rf_fric_mammals_df$ses,
                             mtryStart = 17,
                             ntreeTry = 300,
                             stepFactor = 1.5,
                             improve = 0.00001,
...
# Compute 100 random forests and mean importance of each variable + ALE plots:
varimp_mammals <- test.rf.model(rf_data = rf_fric_mammals_df,
                                iteration_nb = 100,
                                metric_nm = "FD_fric",
                                taxa_nm = "MAMMALS",
                                drivers_nm_df = drivers_nm_df,
...
# Check types of variables
str(rf_fric_butterflies_df)

# Check that diversity metric doesn't have NA:
rownames(rf_fric_butterflies_df[which(is.na(rf_fric_butterflies_df$ses) == TRUE), ])

# Change SES from names num to num:
rf_fric_butterflies_df$ses <- as.numeric(rf_fric_butterflies_df$ses)

# Set seed for randomisation:
set.seed(42)

# See if 300 butterflies and mtry = 17 ok:
# Run the random forest model mtry = 17 and 300 butterflies:
rf_butterflies <- randomForest::randomForest(ses~.,
                                             data = rf_fric_butterflies_df,
                                             ntree = 300,
                                             mtry = 17,
                                             importance = TRUE)
# ntree:
...
# mtry:
mtry <- randomForest::tuneRF(rf_fric_butterflies_df[-ncol(rf_fric_butterflies_df)],
                             rf_fric_butterflies_df$ses,
                             mtryStart = 17,
                             ntreeTry = 300,
                             stepFactor = 1.5,
                             improve = 0.00001,
...
# Compute 100 random forests and mean importance of each variable + ALE plots:
varimp_butterflies <- test.rf.model(rf_data = rf_fric_butterflies_df,
                                    iteration_nb = 100,
                                    metric_nm = "FD_fric",
                                    taxa_nm = "BUTTERFLIES",
                                    drivers_nm_df = drivers_nm_df,
...
ggplot2::ggsave(plot = varimp_plot_butterflies,
                filename = here::here("outputs",
                                      "varimp_FD_fric_50_BUTTERFLIES.pdf"),
                device = "pdf",
                scale = 1,
                height = 5000,
                width = 8000,
...
# Link the two tables (drivers + diversity):
var_fric_birds_df <- dplyr::left_join(envdriv_full_db,
                                       fric_ses_birds_df[, c("Idgrid", "ses")],
                                       by = "Idgrid")
var_fric_trees_df <- dplyr::left_join(envdriv_full_db,
                                       fric_ses_trees_df[, c("Idgrid", "ses")],
                                       by = "Idgrid")
var_fric_reptiles_df <- dplyr::left_join(envdriv_full_db,
                                          fric_ses_reptiles_df[, c("Idgrid", "ses")],
                                          by = "Idgrid")

# Put Idgrid as rownames:
var_fric_birds_df <- var_fric_birds_df %>%
  tibble::column_to_rownames(var = "Idgrid")
var_fric_trees_df <- var_fric_trees_df %>%
  tibble::column_to_rownames(var = "Idgrid")
var_fric_reptiles_df <- var_fric_reptiles_df %>%
  tibble::column_to_rownames(var = "Idgrid")
...
direction_plots <- relationships.plot(ses_var_df = var_fric_birds_df,
                                      metric_nm =  "FRic FD - Birds",
                                      palette =  c("#88CCEE",
                                                   "#44AA99",
                                                   "#117733",
...
ggplot2::ggsave(plot = direction_plots$past_stab,
                filename = here::here("outputs",
                                      "direction_past_stab_FD_fric_50_BIRDS.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$present_hab,
                filename = here::here("outputs",
                                      "direction_pres_hab_FD_fric_50_BIRDS.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$present_hab_heterog,
                filename = here::here("outputs",
                                      "direction_pres_hab_het_FD_fric_50_BIRDS.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$disturb,
                filename = here::here("outputs",
                                      "direction_disturb_FD_fric_50_BIRDS.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$past_lu1,
                filename = here::here("outputs",
                                      "direction_past_lu1_FD_fric_50_BIRDS.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$past_lu2,
                filename = here::here("outputs",
                                      "direction_past_lu2_FD_fric_50_BIRDS.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$pr_hum_imp1,
                filename = here::here("outputs",
                                      "direction_pr_hum_imp1_FD_fric_50_BIRDS.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$pr_hum_imp2,
                filename = here::here("outputs",
                                      "direction_pr_hum_imp2_FD_fric_50_BIRDS.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
# ... category to see in which direction it impacts diversity values:
# ... simply plot the data and try to fit a linear model
direction_plots <- relationships.plot(ses_var_df = var_fric_reptiles_df,
                                      metric_nm =  "FRic FD - Reptiles",
                                      palette =  c("#88CCEE",
                                                   "#44AA99",
                                                   "#117733",
...
ggplot2::ggsave(plot = direction_plots$past_stab,
                filename = here::here("outputs",
                                      "direction_past_stab_FD_fric_50_REPTILES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$present_hab,
                filename = here::here("outputs",
                                      "direction_pres_hab_FD_fric_50_REPTILES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$present_hab_heterog,
                filename = here::here("outputs",
                                      "direction_pres_hab_het_FD_fric_50_REPTILES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$disturb,
                filename = here::here("outputs",
                                      "direction_disturb_FD_fric_50_REPTILES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$past_lu1,
                filename = here::here("outputs",
                                      "direction_past_lu1_FD_fric_50_REPTILES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$past_lu2,
                filename = here::here("outputs",
                                      "direction_past_lu2_FD_fric_50_REPTILES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$pr_hum_imp1,
                filename = here::here("outputs",
                                      "direction_pr_hum_imp1_FD_fric_50_REPTILES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$pr_hum_imp2,
                filename = here::here("outputs",
                                      "direction_pr_hum_imp2_FD_fric_50_REPTILES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
# ... category to see in which direction it impacts diversity values:
# ... simply plot the data and try to fit a linear model
direction_plots <- relationships.plot(ses_var_df = var_fric_trees_df,
                                      metric_nm =  "FRic FD - Trees",
                                      palette =  c("#88CCEE",
                                                   "#44AA99",
                                                   "#117733",
...
ggplot2::ggsave(plot = direction_plots$past_stab,
                filename = here::here("outputs",
                                      "direction_past_stab_FD_fric_50_TREES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$present_hab,
                filename = here::here("outputs",
                                      "direction_pres_hab_FD_fric_50_TREES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$present_hab_heterog,
                filename = here::here("outputs",
                                      "direction_pres_hab_het_FD_fric_50_TREES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$disturb,
                filename = here::here("outputs",
                                      "direction_disturb_FD_fric_50_TREES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$past_lu1,
                filename = here::here("outputs",
                                      "direction_past_lu1_FD_fric_50_TREES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$past_lu2,
                filename = here::here("outputs",
                                      "direction_past_lu2_FD_fric_50_TREES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$pr_hum_imp1,
                filename = here::here("outputs",
                                      "direction_pr_hum_imp1_FD_fric_50_TREES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
...
ggplot2::ggsave(plot = direction_plots$pr_hum_imp2,
                filename = here::here("outputs",
                                      "direction_pr_hum_imp2_FD_fric_50_TREES.jpeg"),
                device = "jpeg",
                scale = 0.8,
                height = 5000,
                width = 8000,
